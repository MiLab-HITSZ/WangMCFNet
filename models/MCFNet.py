...
# from FADC引入自适应感受野和自适应卷积核大小机制 import HPFD
# from FDConv import *
# from SMFA013 import *
from HPFD import *
from VGFM import *
from FDConv import *
from MambaBranch.model import *
# from RestorMixer.model2 import *
...
class MCFNet(nn.Module):
    def __init__(self, model_cfg):
        ...
        self.channel_adapter = nn.Conv2d(3, 2, 1)

        # class EASE_oldenc_fcmfree_fdc_fdjump(nn.Module):

        self.easa_b2 = VGFM(dim=32 * 2)
        self.easa_b5 = VGFM(dim=160 * 2)
        self.easa_b7 = VGFM(dim=448 * 2)
        self.align_b2 = nn.Conv2d(3, 32, 1)  # 160x160 -> 80x80
        self.align_b5 = nn.Conv2d(6, 160, 1)  # 320x320 -> 160x160
        self.align_b7 = nn.Conv2d(12, 448, 1)  # 80x80 -> 40x40
        self.FADC0 = HPFD(in_channels=3, out_channels=3, kernel_size=3, stride=1, fs_cfg={'k_list': [3, 5, 7]})
        self.convF1 = nn.Sequential(
            FDConv(32, 160, 1, 1, 0),
            nn.BatchNorm2d(160),
            nn.ReLU(inplace=True),
        )
        self.avgpoolF1 = nn.AdaptiveAvgPool2d((24, 24))
 
        self.encoder = Encoder(base_channel=3)
        self.avgpoolF2 = nn.AdaptiveMaxPool2d((12, 12))  # 输出是 (32,32)
        self.convF2 = nn.Sequential(
            FDConv(160, 448, 1, 1, 0),
            nn.BatchNorm2d(448),
            nn.ReLU(inplace=True),
        )
    ...
    def extract_features(self, x, layers, start_idx=0, end_idx=None, encoder_features=None):
        assert start_idx >= 0
        if start_idx == 0:
        ...
        for idx in range(start_idx, end_idx):
            ...
            if stage:
                ...
                if stage == 'b2':
                    enc_b2 = self.align_b2(encoder_features[0])  # [B, 3, 320, 320] -> [B, 24, 320, 320]
                    enc_b2 = F.interpolate(enc_b2, size=(96, 96), mode='bilinear', align_corners=False)
                    att1 = self.easa_b2(x, enc_b2)
                    x = x + att1
                    res25 = self.avgpoolF1(self.convF1(att1))

                if stage == 'b5':
                    enc_b5 = self.align_b5(encoder_features[1])  # [B, 3, 320, 320] -> [B, 24, 320, 320]
                    enc_b5 = F.interpolate(enc_b5, size=(24, 24), mode='bilinear', align_corners=False)
                    att2 = self.easa_b5(x, enc_b5)
                    x = x + att2 + res25
                    res57 = self.avgpoolF2(self.convF2(att2))

                if stage == 'b7':
                    enc_b7 = self.align_b7(encoder_features[2])  # [B, 3, 320, 320] -> [B, 24, 320, 320]
                    enc_b7 = F.interpolate(enc_b7, size=(12, 12), mode='bilinear', align_corners=False)

                    att1 = self.easa_b7(x, enc_b7)
                    x = x + att1 + res57
    ...
    def forward(self, samples):
        ...
        x = self.extract_features(x, layers, encoder_features=features)
        if x is None:
            return layers
        layers['final'] = x
